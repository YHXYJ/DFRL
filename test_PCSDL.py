...
import torch
# model.model_DNANet import DNANet
#from model.model_DNANet import Res_CBAM_block
from model.PCSDL.MSHNet import *
...
#model_dir=f'E:/vipl-ll/1_code/11_PBT/1_diff_wave/runs/Feb22_16-01-38_NUDT-SIRST_6_4_PBT/checkpoint/Best_nIoU_Epoch-1386_IoU-0.9374_nIoU-0.9371.pth.tar'
model_dir=r'G:\vipl_ll\1_code\0_icassp2026_code\pth\PCSDL\Best_PCSDL.pth'
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='NUAA-SIRST',
                        help='dataset:IRSTD-1k; NUAA-SIRST; NUDT-SIRST; IRSTD-Air')
    parser.add_argument('--split_method', type=str, default='80_20',help='6_4')
...
class PD_FA():
    ...
    def get(self,img_num):
        ...
        Final_FA =  self.FA / ((256*256) * img_num)#512
...
def visual(model, data_loader, device, epoch, iou_metric, nIoU_metric, PD_FA, mIoU, ROC, len_val, path, ids, suffix, dataset_dir, test_txt, size):
    ...
    for step, data in enumerate(data_loader):
        ...
        pred = model(images.to(device), False)
        ...
        if isinstance(pred, list):
            ...
            pass
        pred0 = torch.sigmoid(pred[1])
        pred0[pred0 > 0.5] = 1
        pred0[pred0 <= 0.5] = 0
        label = labels.float()
        label[label > 0] = 1
        print(f"[DEBUG] idx={step}, pred0.shape={pred0.shape}, label.shape={label.shape}")
        ...
        pred, labels = pred[1].cpu(), labels.cpu()
